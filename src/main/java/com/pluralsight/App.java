...
public class App {
    public static void main(String[] args) {
        ...
        dataSource.setPassword("MySQL101yearup");

        DataManager myManager = new DataManager(dataSource);
        List<Actor> listOfAllActors = myManager.getActors(lastName);
        System.out.printf("""
                
                Here are the list of actors with id and name based of selected last name:
        ...
        System.out.println("Enter the id number to look up for the movies where the actor has been in: ");
        int answer = scanner.nextInt();

        List<Film> ListOfAllMovies = myManager.getAllFilms(answer);
        System.out.println("Here is the list of movies with the details: ");

        for (Film film: ListOfAllMovies) {
